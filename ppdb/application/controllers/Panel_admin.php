...
class Panel_admin extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->session->userdata('id_admin') == NULL) {
			...
			// $data['user'] = $this->db->get_where('tbl_user', ['username' => $sess])->row_array();
			// var_dump($this->session->userdata('administrator'));die;
	...
	public function cetak_berkas($id = null){
		$sess = $this->session->userdata('id_admin');
		if ($sess == NULL) {
			redirect('panel_admin/log_in');
		}

		$data['siswa'] = $this->db->get_where('tbl_siswa', ['no_pendaftaran' => $id])->row_array();
		$data['berkas'] = json_decode($data['siswa']['dokumen']);
		// var_dump($berkas->ijazah);die;
		//
		// if (!$data['berkas']) {
		// 	// code...
		// 	echo "kosong";
		...
		// }
		// die;
		$this->load->view('admin/verifikasi/cetakberkas', $data);
	}
